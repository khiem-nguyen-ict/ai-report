...
async function waitForStreamingComplete(page) {
    ...
    throw new Error(`❌ Timed out after ${STREAMING_TIMEOUT_MS / 1000}s waiting for generation.`);
}
...
async function sendToClaudeAndDownload(prompt, outputPath) {
    ...
    try {
        ...
        await page
            .screenshot({ path: path.join(__dirname, "../../app-data/debug-no-download-btn.png"), fullPage: false })
            .catch(() => { });
        throw new Error(
    ...
    await page
        .screenshot({ path: path.join(__dirname, "../../app-data/debug-before-download.png"), fullPage: false })
        .catch(() => { });
